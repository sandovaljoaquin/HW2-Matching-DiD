...
Stokes used distance buffers to see how far the spillover effect extends. To mitigate spillover bias, what would have been control precincts within 3 kilometers of a precinct with a turbine are excluded from the control group.
...
Treatment indicator: binary variable showing which units received treatment (e.g.`proposed_turbine_3km`)

Outcome variable: Dependent variable the study is measuring (e.g.`change_liberal`)

Observed covariates: Pre-treatment variables to match on (e.g. `log_home_val_07`, `p_uni_degree`, `log_median_inc`, `log_pop_denc`)
...
Yes, we've identified a causal estimate using this approach. The estimated effect of treatment on change in liberal party vote share is -6%. 
...
No, it is 1.858 to 1 (Absolute value of the t statistic) which is below the 2 to 1 threshold.
